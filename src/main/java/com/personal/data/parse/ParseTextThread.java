...
public class ParseTextThread implements Runnable {
	...
	public ParseTextThread(CountDownLatch latch, BlockingQueue<Article> queue, int begin, int end) {
		this.latch = latch;
		this.queue = queue;
		this.begin = begin;
		this.end = end;
	...
	public void run() {
		...
		for (int i = begin; i <= end; i++) {
			try {
				doc = Jsoup.connect(DataHelper.PARSE_URL + i).userAgent(DataHelper.USER_AGENT)
				...
				for (Element el : els) {
					...
					queue.offer(article);
					System.out.println("parse article title -> " + article.getTitle());
