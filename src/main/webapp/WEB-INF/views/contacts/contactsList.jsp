				...
				<tbody>
					<c:forEach var="contact" items="${contacts.content }">
						<tr>
							...
							<td><a href="${ctx }/contacts?search_EQ_type.id=${contact.type.id}">${contact.type.name }</a></td>
							...
							<a href="${ctx}/contacts/delete/${contact.id}" onclick="return confirm('确认删除?');" title="删除"><i class="icon-trash"></i></a>
							</td>
						</tr>
					</c:forEach>
				</tbody>
