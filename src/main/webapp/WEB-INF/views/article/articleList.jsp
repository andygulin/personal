				...
				<tbody>
					<c:forEach var="article" items="${articles.content }">
						<tr>
							...
							<td><a href="${ctx }/article?search_EQ_type.id=${article.type.id}">${article.type.name }</a></td>
							<td><c:if test="${empty article.srcName }"><span style="color:#CCC;">无图说个JB</span></c:if><c:if test="${not empty article.srcName }"><b>有图有真相</b></c:if></td>
							...
							<a href="${ctx}/article/delete/${article.id}" onclick="return confirm('确认删除?');" title="删除"><i class="icon-trash"></i></a>
							</td>
						</tr>
					</c:forEach>
				</tbody>
