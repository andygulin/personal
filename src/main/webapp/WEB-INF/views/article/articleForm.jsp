	...
	<form id="articleForm" action="${ctx}/article/${action}" method="post" class="form-horizontal">
		...
		<fieldset>
			...
			<div class="control-group">
				...
				<div class="controls">
					<input type="text" id="title" name="title"  value="${article.title}" class="input-large required" style="width:500px;" />
				</div>
			</div>	
			<div class="control-group">
				...
				<div class="controls">
					...
					<c:forEach var="at" items="${articleType }">
        				<option <c:if test="${(not empty article.type.id) and (article.type.id eq at.id)}">selected="selected"</c:if> value="${at.id }">${at.name }</option>
        			</c:forEach>
    				</select>
				</div>
			</div>	
