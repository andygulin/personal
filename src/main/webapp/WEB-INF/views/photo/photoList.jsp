            ...
            <div class="span3" style="padding-top: 20px; padding-bottom: 20px;">
                <c:if test="${photoType.count>0 }">
                    <a href="${ctx }/photo/view/${photoType.id}" title="${photoType.name }"><img
                            class="img-polaroid" src="${ctx }/photo/printImage/${photoType.cover.id}"
                            style="width:213px;height:160px"/></a>
                </c:if>
                <c:if test="${photoType.count==0 }">
                ...
                <p class="text-center" style="margin-top:5px;">
                    ...
                    <c:if test="${photoType.count>0 }"><a href="${ctx }/photo/download/${photoType.id}" title="打包下载该相册"><i
                            class="icon-share"></i></a></c:if>
                    <c:if test="${!photoType.isDefault }">
                        <a href="${ctx }/photo/updateType/${photoType.id}" title="修改相册名称"><i
                                class="icon-pencil"></i></a>
                    </c:if>
                    <c:if test="${photoType.count>0 }"><a href="javascript:;" onclick="deleteAll('${photoType.id}');"
                                                          title="清空该相册"><i class="icon-remove"></i></a></c:if>
                </p>
            </div>
        </c:forEach>
    </div>
...
<script type="text/javascript">
    function deleteAll(id) {
        if (confirm("确认清空?")) {
            $.post("${ctx}/photo/removeAll/" + id, {d: new Date().getTime()}, function (data) {
