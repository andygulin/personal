...
<div class="container">
	...
	<p class="text-right">
		...
		<select id="selectPhotoType" style="width:200px;">
			<c:forEach var="pt" items="${photoTypes }"><option <c:if test="${pt.id eq id }">selected="selected"</c:if> value="${pt.id }">${pt.name }(${pt.count })</option></c:forEach>
		</select>
	</p>
	<div class="row">
		<c:forEach var="pt" items="${photos.content }">
			<span id="photo_${pt.id }" class="span4">
				...
				<img title="${pt.remark }" class="img-polaroid" src="${ctx }/photo/printImage/${pt.id}" style="width:300px;height:225px" /></a></p>
